...
class EncodeLayer(tf.keras.layers.Layer):
    ...
    #   mask (..., seq_length_v) broadcast to (..., seq_length_q, seq_length_v)
        ...
        x = self.layernorm1(self.dropout1(x + attention_outputs, training=training), training=training)
        ff = self.feed_forward(x) # (..., seq_lenght, d_model)
        x = self.layernorm2(self.dropout2(x + ff, training=training), training=training)
        return x
...
class DecodeLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, encoder_output, padding_mask, look_ahead_mask, training=None):
        self_attention_outputs = self.self_attention(x, x, x, look_ahead_mask) # (..., seq_length_decoder, d_model)
        x = self.layernorm1(self.dropout1(x + self_attention_outputs, training=training), training=training)
        ...
        x = self.layernorm2(self.dropout2(x + encoder_attention_outputs, training=training), training=training)
        ff = self.feed_forward(x) # (..., seq_length_decoder, d_model)
        x = self.layernorm3(self.dropout3(x + ff, training=training), training=training)
        return x
...
class Decoder(tf.keras.layers.Layer):
    ...
    # encoder_putput (..., seq_length_source, d_model)
        ...
        x = self.dropout(x, training=training)
        for i in range(self.num_layers):
            x = self.layers[i](x, encoder_output, padding_mask, look_ahead_mask, training=training)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        inp, tar = inputs
        encoder_padding_mask, decoder_padding_mask, look_ahead_mask = create_masks(inp, tar)
        encoder_output = self.encoder(inp, encoder_padding_mask, training=training)
        # encoder_output (batch_size, source_seq_length)
        decoder_output = self.decoder(tar, encoder_output, decoder_padding_mask, look_ahead_mask, training=training)
